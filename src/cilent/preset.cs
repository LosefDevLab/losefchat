...
public partial class Preset
{
    ...
    public string presetFilePath = "preset.txt";
    public void ReadPreset() // 读取预设
    {
        if(!File.Exists(presetFilePath))
        {
            using (File.Create(presetFilePath)) { }
            Console.WriteLine("您的预设文件第一次建立或者丢失了, 请填写预设文件信息,注意每一行都要小心填放,不能有行数缺失或者乱放");
            Console.WriteLine("预设置文件放在Losefchat主程序同目录下，名称为preset.txt");
            Console.WriteLine("请按照以下格式填写:");
            Console.WriteLine("第一行:您的IP协议");
            Console.WriteLine("第二行:服务器IP地址");
            Console.WriteLine("第三行:端口号");
            Console.WriteLine("第四行:用户名(为空就使用计算机名称)");
        ...
        string[] lines = File.ReadAllLines(presetFilePath);
        if (lines.Length >= 4)
        {
